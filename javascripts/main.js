...
function renderPage(json) {
  ...
  json.forEach(function (ele) {
    html += `<li class ="box">
            <a href="#">
              <img src = "${ele.show.img}" width=${220} height=${260}  alt="">
              <p >${ele.title}</p>
              <div class="line"></div>
...
function isload() {
  // 关键参数不存在，函数不执行；
  var st = document.body.scrollTop || document.documentElement.scrollTop; // var mh = Math.min.apply(flase,)
...
function handleCarClick(event) {
  ...
  var nowMsg = findJson(iid)[0]; //   console.log(nowMsg)
...
function addCar(nowMsg, iid) {
  ...
  var sNowMsg = JSON.stringify(nowMsg); //   console.log(sNowMsg)
  // console.log(sNowMsg);
  ...
  if (sNowMsg) {
    localStorage.cart = JSON.parse(sNowMsg);
    return false;
  } // 如果存在对结构进行插入;


  var aMsg = localStorage.cart; //   console.log(aMsg)
  ...
  localStorage.cart = JSON.stringify(aMsg);
...
$(".go-buy").on("mouseenter", function () {
  $(".car-list").show();
  console.log(getCart());
  $(".car-list ul").html(renderCart());
});
$(".go-buy").on("mouseleave", function () {
  $(".car-list").hide();
});
...
// 轮播图：
function banner() {
  var color = [[205, 65, 64], [205, 65, 64], [205, 65, 64], [205, 65, 64], [74, 116, 180]];

  function Banner() {}

  $.extend(Banner.prototype, {
    init: function (options) {
      this.box = $(options.box); // 图片列表

      this.item_list = $(options.item_list); // 按钮列表

      this.list_btn = $(options.list_btn); // 图片列表的父级；

      this.list = this.item_list.parent(); // 元素的宽度

      this.ele_width = $(options.ele_width)[0];
      this.parentBox = $(options.parentBox); // 当前图片的下标

      this.nowIndex = 0; // 图片的总数

      this.item_num = this.item_list.length; // 如果轮播图没有任何按钮，就不调用绑定事件方法了； 可以直接自动播放；
    ...
    toIndex: function (e) {
      ...
      this.nowIndex = $(target).index(); // console.log(this.nowIndex);
